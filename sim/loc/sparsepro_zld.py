...
def zld(args):
    ...
    for ite in range(len(ldlists)):
        ...
        if args.h2:
            ...
            h2_hess, var_b, p_chi, K=get_HESS_h2_z(LD.values, z[1].values, args.N, gwp=args.gwp)
        if args.K is not None:
            K = args.K
        if args.anno is not None and args.aW is not None:
            annofile = ldlists.at[ite,args.anno]
            ianno = pd.read_csv(annofile,sep='\t',index_col=0)
    ...
    if args.anno is not None and args.aW is None:
        annomat = pd.concat([pd.read_csv(i,sep='\s+',index_col=0) for i in ldlists[args.anno]])
...
parser.add_argument('--sthres', type=float, default=0.95, help='level for credible sets')
