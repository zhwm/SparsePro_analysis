...
def match(ss, args, ch):
    ...
    ol = len(sub)
    sub = sub.loc[
        ((sub['A2'] == idx['A2']) & (sub['A1'] == idx['A1'])) | ((sub['A2'] == idx['A1']) & (sub['A1'] == idx['A2']))]
...
def merge(args):
    ...
    if not args.ukb:
        ...
        SS = [preprocess_ukb(i) for i in SS]
        for ch in range(1, 23):
            matched = [match_with_ukb(i, args, ch) for i in SS]
            mid = pd.concat([i['SNP'] for i in matched], axis=1, join='inner').index
            if len(mid) == 0:
                continue
            [matched[i].loc[mid].to_csv(os.path.join(args.save, "{}_{}.z".format(args.prefix[i], ch)), sep='\t',
                                        header=False, index=False) for i in range(len(matched))]
...
parser.add_argument('--ukb', action='store_true',
                    help='option for processing UK Biobank summary statistics from Neale lab')
