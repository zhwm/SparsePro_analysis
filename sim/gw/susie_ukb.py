...
def ukb(args):
    ...
    for ite in range(len(ldlists)):
        ...
        if effnum <=10:
            ...
            continue
        h2_hess, var_b, p_chi, K=get_HESS_h2_z(LD.values, z.loc[idx,1].values, args.N)
        ...
        if args.K is not None:
            K = args.K
        susierss = susieR.susie_rss(z.loc[idx,1].values.reshape(-1,1),LD.values,L=K,n=args.N)
